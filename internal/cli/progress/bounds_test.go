...
func TestProgressChangeTimeTracking(t *testing.T) {
	...
	// Sleep before the first update so that elapsed time is measurable even on
	// platforms with coarse timer resolution (e.g. Windows ~15ms tick).
	...
	// Wait a bit and update again. Sleep for 100ms to ensure the elapsed time
	...
	// Elapsed time should have increased beyond a small threshold. Using 10ms as
	// the lower bound leaves plenty of margin below the 100ms sleep duration.
