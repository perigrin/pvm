...
func TestCheckFileVerboseMode(t *testing.T) {
	...
	errorCount, err := checkFile(ui.NewDefaultOutput(), tmpFile, false, true, false, false)
	assert.NoError(t, err)
...
func TestCheckFileStrictMode(t *testing.T) {
	...
	errorCount, err := checkFile(ui.NewDefaultOutput(), tmpFile, true, false, false, false)
	assert.NoError(t, err)
...
func TestMixedFileTypes(t *testing.T) {
	...
	errorCount, err := checkFile(ui.NewDefaultOutput(), tmpDir+"/test.pl", false, false, false, false)
	assert.NoError(t, err)
	assert.Equal(t, 0, errorCount)

	// Test that non-Perl files are skipped
	errorCount, err = checkFile(ui.NewDefaultOutput(), tmpDir+"/readme.txt", false, false, false, false)
	assert.NoError(t, err)
...
func TestFilePermissions(t *testing.T) {
	...
	_, err = checkFile(ui.NewDefaultOutput(), restrictedFile, false, false, false, false)
