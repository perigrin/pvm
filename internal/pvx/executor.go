...
func ExecuteScript(options *ExecutionOptions, uiOutput ...*ui.Output) (string, error) {
	...
	// Auto-detect dependencies using regex parsing if enabled
	if options.AutoDetectDependencies {
		...
		// Dependency detection uses regex heuristics (AST-based detection not yet available)
	...
	// Module auto-installation is not yet available in this build
	if options.AutoInstallModules && len(options.RequiredModules) > 0 {
		if uiOut != nil {
			uiOut.Warning("Module auto-installation is not yet available in this build; skipping install of %d modules", len(options.RequiredModules))
		} else if options.Verbose {
			log.Warnf("Module auto-installation is not yet available in this build; skipping install of %d modules", len(options.RequiredModules))
		}
	}

	// Check if script contains type annotations and handle PSC type checking if needed
	// Type checking and annotation stripping are not yet available in this build.
	// ContainsTypeAnnotations still detects annotations via regex heuristics.
	if ContainsTypeAnnotations(options.ScriptPath) {
		if (options.TypeCheck || options.FlowSensitive) && !options.SkipTypeCheck {
			if uiOut != nil {
				uiOut.Warning("Type checking is not yet available in this build; skipping type check")
			} else if options.Verbose {
				log.Warnf("Type checking is not yet available in this build; skipping type check")
			}
		}

		if options.Verbose {
			if uiOut != nil {
				uiOut.Warning("Type annotation stripping is not yet available in this build; running script as-is")
			} else {
				log.Warnf("Type annotation stripping is not yet available in this build; running script as-is")
			}
		}
...
// stripTypeAnnotationsFromScript is a stub — type annotation stripping is not yet available.
// Returns the original script path unchanged with a no-op cleanup function.
func stripTypeAnnotationsFromScript(scriptPath string) (string, func(), error) {
	log.Warnf("Type annotation stripping is not yet available; executing script as-is: %s", scriptPath)
