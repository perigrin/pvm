...
// newAdvancedResolverForTest creates an *advancedResolver directly for tests
// that need access to concrete methods not exposed by the DependencyResolver interface.
func newAdvancedResolverForTest(t *testing.T, cacheDir string, cacheTTL int) *advancedResolver {
	t.Helper()
	baseResolver, err := NewDefaultResolver(cacheDir, cacheTTL)
	if err != nil {
		t.Fatalf("Failed to create base resolver: %v", err)
	}
...
func TestAdvancedResolver_ConflictResolution(t *testing.T) {
	...
	advancedResolver := newAdvancedResolverForTest(t, "", 0)
