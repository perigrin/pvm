...
func TestGetRequiredModulesForScript_ErrorCases(t *testing.T) {
	...
	t.Run("InvalidPerlSyntax", func(t *testing.T) {
		// Create a temp file with syntactically unusual content
		...
		// Write content that a Perl parser would reject (unclosed string)
		// Regex-based extraction will still attempt to extract what it can
		...
		// The function should handle any file gracefully without errors
		// (regex-based parsing does not detect syntax errors)
		_, err = GetRequiredModulesForScript(tmpfile.Name())
		if err != nil {
			t.Errorf("Function should not return an error for any readable file, but got: %v", err)
		}
	})
}
