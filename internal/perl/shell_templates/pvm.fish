...
# Tell pvm subprocesses which shell they were launched from. $SHELL reflects
# the user's login shell and may mislead pvm when fish sources this
# integration.
set -gx PVM_SHELL fish
...
function pvm_init
    ...
    function pvm
        ...
        if test "$command" = "use"
            ...
            eval (command "$pvm_exec" sh-use "$requested_version")
...
if test "$PVM_SKIP_NETWORK_CALLS" != "1"
    ...
    if test -n "$_pvm_exec_path"
        set -l _pvm_completion ("$_pvm_exec_path" completion fish 2>/dev/null)
        if test -n "$_pvm_completion"
            eval "$_pvm_completion"
        end
        set -e _pvm_completion
    end
