...
func TestGenerateShims(t *testing.T) {
	...
	expectedShims := []string{"perl", "cpan", "prove", "perldoc"}
	for _, shimName := range expectedShims {
		shimPath := filepath.Join(dirs.ShimsDir, shimName)
		if runtime.GOOS == "windows" {
			shimPath += ".bat"
		}

		if _, err := os.Stat(shimPath); os.IsNotExist(err) {
			t.Errorf("Expected shim %s was not created at %s", shimName, shimPath)
			continue
		}
		...
		content, err := os.ReadFile(shimPath)
		if err != nil {
			t.Errorf("Failed to read shim %s: %v", shimName, err)
			continue
		}

		contentStr := string(content)
		if !contains(contentStr, "Generated by PVM") {
			t.Errorf("Shim %s does not contain 'Generated by PVM' header", shimName)
		}

		if !contains(contentStr, pvmPath) {
			t.Errorf("Shim %s does not contain PVM path: %s", shimName, pvmPath)
		}
	}
}
