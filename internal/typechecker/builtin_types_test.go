...
func TestBuiltinTypeRegistry_BasicFunctions(t *testing.T) {
	...
	testCases := []struct {
		...
		{"keys", 1, "ArrayRef[Str]", "keys() should return ArrayRef[Str]"},
		{"values", 1, "ArrayRef[Any]", "values() should return ArrayRef[Any]"},
		...
		{"split", 2, "ArrayRef[Str]", "split() should return ArrayRef[Str]"},
