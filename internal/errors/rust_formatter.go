...
func (rsf *RustStyleFormatter) FormatTreeSitterError(errNodes []ErrorNodeInfo, filePath, sourceContent string) string {
	...
	for i, errNode := range errNodes {
		...
		position := Position{Line: lineNum, Column: colNum}
...
func (rsf *RustStyleFormatter) formatErrorLocation(position Position, filePath string) string {
	var result strings.Builder
...
func (rsf *RustStyleFormatter) formatSourceContext(sourceLine string, position Position) string {
