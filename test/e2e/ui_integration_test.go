...
func TestUIFramework_ErrorHandling(t *testing.T) {
	...
	errorTests := []struct {
		...
		},
		{
			...
			command:     []string{"psc", "parse", "/nonexistent/file.pl"},
			expectError: true,
		},
	}
...
`
	...
	t.Run("Help consistency", func(t *testing.T) {
		...
		for _, subcmd := range subcommands {
			...
			// All help should contain Usage information (styled output uses USAGE, cobra uses Usage:)
			hasUsage := strings.Contains(stdout, "Usage:") || strings.Contains(stdout, "USAGE")
