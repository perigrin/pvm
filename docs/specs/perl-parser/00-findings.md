...
**WRONG is three, and all three are one defect in our grammar.**
`mro/package_aliases.t`, `mro/package_aliases_utf8.t` and `op/filetest.t`
each contain a quote-like construct our grammar never closes —
`package_aliases.t`'s `q~ ... ~` at line 53 — after which the rest of the
file, backslashes included, is one string literal with no error node. Perl
took six, five and four references in those regions; the subject's tree
reports none, hedges none, and says nothing. That is a tree that stopped
being a parse of its source, exactly what the degenerate gate in
`adapter.go` exists to decline, and a shape it does not yet detect.
Whole-file hedging was hiding all three as `wider`; per-statement scoring
is what surfaced them. The bucket is doing its job. `op/filetest.t`'s
swallow begins between lines 100 and 120 and was not pinned further.
